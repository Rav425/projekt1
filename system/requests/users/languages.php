...
if(isset($_POST["search"]["value"]) && !empty($_POST["search"]["value"])){  // SEARCH DATA..	
	$query .= 'WHERE l_key LIKE "%'.$_POST["search"]["value"].'%"
        OR en_value = "%'.$_POST["search"]["value"].'%"
        OR de_value = "%'.$_POST["search"]["value"].'%"
	';
}
...
while($row = $result->fetch_array()){
    ...
    $sub_array[] = '<div style="text-align:center;font-weight:bold">' . $row["l_id"] . '</div>';
    $sub_array[] = '<div style="text-align:center;">' . $row["l_key"] . '</div>';
    $sub_array[] = '<div style="text-align:center;">' . $row["en_value"] . '</div>';
    $sub_array[] = '<div style="text-align:center;">' . $row["de_value"] . '</div>';
    
	$sub_array[] = '
        <div style="text-align:center;">
            <a href="#" cid="'.$row["l_id"].'"class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1 editthis" title="Edit Word">
            ...
            <a href="#" cid="'.$row["l_id"].'"class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1 deletethis" title="Delete Word">
                <span class="svg-icon svg-icon-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
